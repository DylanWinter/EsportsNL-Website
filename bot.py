...
async def get_stats(interaction: discord.Interaction, user_str: str = None):
    if user_str:
        user_ids = parse_users(user_str)
        if not user_ids:
            await interaction.response.send_message("Couldn't parse any users from your input.")
            return
        discord_id = user_ids[0]
    else:
